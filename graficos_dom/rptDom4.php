...
?>
  ...
  <style type="text/css">
              ...
              data: [
                <?php
          			$sql=mysql_query("SELECT a.tdoc_pac,COUNT(doc_pac) cuantos,nom_completo,TIMESTAMPDIFF(YEAR,fnacimiento,CURDATE()) AS edad,
                        ...
                        GROUP BY calificacion_total");
          			while($res=mysql_fetch_array($sql)){
          			?>
          ['<?php echo $res['calificacion_total'].' ('.$res['cuantos']?>)', <?php echo $res['cuantos'] ?> ],
          			<?php
          			}
          			?>
              ]
	...
	<script type="text/javascript">
        ...
        <section class="modal fade" id="graficosbarthel" role="dialog">
          <section class="modal-dialog modal-lg" >
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h3 class="modal-title alert alert-success">Consolidado de resultados Escala BARTHEL</h3>
              </div>
