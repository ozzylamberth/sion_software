...
class MYPDF extends TCPDF {
	public function Header() {
        ...
        $date1=date('m');
        $y=date('Y');
        $this->multicell(40,10,$this->image($image_file, $this->GetX(), $this->GetY(),60,30),0,'J');
        if ($date1==1) {
          $mes='Enero';
          }
          if ($date1==2) {
        ...
        // Title
        $this->Cell(0, 15, 'Evolución de control Salud mental', 1, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln();
	...
	public function ColoredTable($header,$data,$data1) {
      ...
      $this->Cell(40,6, utf8_encode('Consulta Externa SM'),1,0,'C');
      ...
      $this->Ln();
      $this->multicell(180, 0,utf8_encode($row['ddxp'] .' '.$row['tdxp'] ), 1, 'J');
      $this->multicell(180, 0,utf8_encode($row['ddx1'] .' '.$row['tdx1'] ), 1, 'J');
      $this->multicell(180, 0,utf8_encode($row['ddx2'] .' '.$row['tdx2'] ), 1, 'J');
            $this->Ln();
...
pacientes a inner join adm_hospitalario b on (b.id_paciente = a.id_paciente)
		    inner join evolucion_medica e on (e.id_adm_hosp = b.id_adm_hosp)
