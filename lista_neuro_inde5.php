...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			break;
			case 'EVO':
				$sql="INSERT INTO evo_neuro_inde (id_adm_hosp, id_user, freg_evo_neuro, hreg_evo_neuro, objetivo_neuro, subjetivo_neuro, analisis_evo_neuro, plan_tratamiento_neuro, dxp, tdxp, dx1, tdx1, dx2, tdx2, servicio, estado_evo_neuro) VALUES
...
<section class="panel-heading"><h4>Consulta Externa Especialidad Neurologia</h4></section>
<div class="panel-body">
	...
	<table class="table table-responsive table-bordered">
		...
		$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,fotopac,
					...
					WHERE p.doc_pac='".$doc."' and a.estado_adm_hosp='Activo' and fingreso_hosp='$f'
		...
		if ($tabla=$bd1->sub_tuplas($sql)){
			foreach ($tabla as $fila ) {
				...
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio=Consulta externa INDE"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-flask"></span></button></a></th>';
			...
			foreach ($tabla as $fila ) {
				if ($fila['tipo_servicio']=='Demencias') {
					...
					echo'<th class="text-right alert alert-success">
								<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=HC&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-plus-circle"></span> Valoración Inicial</button></a></p>
								<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
								<p><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio=Consulta externa INDE"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-flask"></span> Ayudas DX</button></a></p>
							 </th>';
					echo "</tr>\n";
				}
				...
				if ($fila['tipo_servicio']=='Consulta externa INDE') {
					...
					echo'<th class="text-right alert alert-danger">
								<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=HC&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-plus-circle"></span> Valoración Inicial</button></a></p>
								<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
								<p><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio=Consulta externa INDE"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-flask"></span> Ayudas DX</button></a></p>
							 </th>';
