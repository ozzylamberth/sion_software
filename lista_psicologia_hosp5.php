...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."','".$_POST["tsesion"]."','".$_POST["objsesion"]."','".$_POST["actividades"]."',
			 '".$_POST["resultado"]."','".$_POST["obsevopsico"]."','Realizada','".$_SESSION["AUT"]["nombre"]."')";
			$subtitulo="Evolución";
			...
			'".$_POST["motivo_consulta"]."','".$_POST["h_familiar_personal"]."','".$_POST["evaluacion_psico"]."',
...
if (isset($_GET["mante"])){					///nivel 2
	switch ($_GET["mante"]) {
		...
		$sql="SELECT a.tdoc_pac,a.doc_pac,nom1,nom2,ape1,ape2,edad,fnacimiento,dir_pac,tel_pac,rh,
			...
			$form1='formulariosHOSP/PSICO/add_evo_psicologia.php';
		break;
      ...
      where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$opcion=$_REQUEST['opcion'];
			$doc=$_REQUEST['doc'];
			$subtitulo='Valoración inicial de psicología';
			$form1='formulariosHOSP/PSICO/val_psicoce_sm.php';
			break;
		}
//echo $sql;
		if($sql!=""){
...
<section class="panel panel-default">
	<section class="panel-heading">
		<h4>Psicología en servicio de SALUD MENTAL</h4>
	</section>
			...
			<section class="col-md-2 text-center">
				<div class="row">
					<div class="col-lg-12">
						<div class="input-group">
							<?php
								echo'<a href="'.PROGRAMA.'?opcion=180"><button type="button" class="btn btn-info" ><span class="fa fa-search fa-2x"></span> Multievolución <br> Psicología</button></a>';
							?>
						</div><!-- /input-group -->
					</div><!-- /.col-lg-6 -->
				</div>
							...
							$adm=$fila['id_adm_hosp'];
										...
										if ($tablav=$bd1->sub_tuplas($sqlv)){
											foreach ($tablav as $filav ) {
												...
												echo'<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>';
											}
										}else {
											echo'<th class="text-left">
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-plus-circle"></span> Valoración <br> Inicial</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
													 </th>';
										}
							echo'</th>';
							echo "</tr>\n";
						...
						if ($fila['tipo_servicio']=='Hospital dia') {
							...
							$adm=$fila['id_adm_hosp'];
										...
										if ($tablav=$bd1->sub_tuplas($sqlv)){
											foreach ($tablav as $filav ) {
												echo'<p class="text-primary">Valoración Registrada</p>';
												echo'<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>';
											...
											echo'<th class="text-left">
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-plus-circle"></span> Valoración <br> Inicial</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
						...
						if ($fila['tipo_servicio']=='Consulta Externa SM' ) {
							...
							$adm=$fila['id_adm_hosp'];
										...
										if ($tablav=$bd1->sub_tuplas($sqlv)){
											...
											echo'<th class="text-left">
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-plus-circle"></span> Valoración <br> Inicial</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio='.$fila['tipo_servicio'].'&doc='.$fila["doc_pac"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-success " ><span class="fa fa-flask"></span> Ayuda Dx</button></a></p>
													 </th>';
										}
				...
				if ($tabla=$bd1->sub_tuplas($sql)){
					...
					foreach ($tabla as $fila ) {
							...
							$adm=$fila['id_adm_hosp'];
										...
										if ($tablav=$bd1->sub_tuplas($sqlv)){
											foreach ($tablav as $filav ) {
												echo'<p class="text-primary">Valoración Registrada</p>';
												echo'<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>';
												echo'<th class="text-center "><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio='.$fila['tipo_servicio'].'&doc='.$fila["doc_pac"].'"><button type="button" class="btn btn-success " ><span class="fa fa-flask"></span> Ayuda Dx</button></a></th>';
											}
										}else {
											echo'<th class="text-left">
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria"><button type="button" class="btn btn-primary" ><span class="fa fa-plus-circle"></span> Valoración <br> Inicial</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>
														<p><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio='.$fila['tipo_servicio'].'&doc='.$fila["doc_pac"].'&atencion=Ambulatoria&sede='.$fila["sede"].'"><button type="button" class="btn btn-success " ><span class="fa fa-flask"></span> Ayuda Dx</button></a></p>
													 </th>';
										}
						...
						if ($fila['tipo_servicio']=='Consulta Externa SM') {
							...
							echo'<th class="text-center">';
							$adm=$fila['id_adm_hosp'];
										$sqlv="SELECT id_psicoce_sm FROM psico_ce_sm where id_adm_hosp=$adm";
										if ($tablav=$bd1->sub_tuplas($sqlv)){
											foreach ($tablav as $filav ) {
												echo'<p class="text-primary">Valoración Registrada</p>';
												echo'<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'&doc='.$fila["doc_pac"].'&servicio='.$fila["tipo_servicio"].'"><button type="button" class="btn btn-primary" ><span class="fa fa-stethoscope"></span> Evolución</button></a></p>';
