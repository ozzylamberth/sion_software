	...
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			switch ($_POST["operacion"]) {
				...
				break;
			case 'CONTROL':
				$sql="INSERT INTO evo_fono_ce (id_adm_hosp, id_user, freg_evofono_ce, hreg_evofono_ce, evolucionfono_ce, estado_evofono_ce) VALUES
...
if (isset($_GET["mante"])){					///nivel 2
    ...
    where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
      ...
      where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$doc=$_REQUEST['doc'];
			$opcion=$_REQUEST['opcion'];
			$form1='formulariosCE/evo_fono_ce5.php';
...
//echo $sql;
		if($sql!=""){
			if (!$fila=$bd1->sub_fila($sql)){
				$fila=array("tdoc_pac"=>"","doc_pac"=>"","nom1"=>"","nom2"=>"","ape1"=>"","ape2"=>"","edad"=>"","fnacimiento"=>"","dir_pac"=>"","tel_pac"=>"","rh"=>"","email_pac"=>"","genero"=>"","lateralidad"=>"",
...
<section class="panel-heading"><h4>FONOAUDIOLOGIA Consulta externa</h4></section>
	...
	if ($tabla=$bd1->sub_tuplas($sql)){
		...
		foreach ($tabla as $fila ) {
			...
			echo'<th class="text-right">';
						...
						$sql_valini="SELECT id_valini_ce,freg,tterapia FROM val_ini_ce WHERE id_adm_hosp=$adm and tterapia='FONOAUDIOLOGIA'
						LIMIT 1";
...
if (isset($_REQUEST["nom"])){
	...
	if ($tabla=$bd1->sub_tuplas($sql)){
		foreach ($tabla as $fila ) {
			...
			echo'<th class="text-right">';
						$adm=$fila['id_adm_hosp'];
						$sql_valini="SELECT id_valini_ce,freg FROM val_ini_ce WHERE id_adm_hosp=$adm and tterapia='FONOAUDIOLOGIA'
						LIMIT 1";
						if ($tabla_valini=$bd1->sub_tuplas($sql_valini)){
							foreach ($tabla_valini as $fila_valini ) {
								echo'<p>Valoración inicial registrada el '.$fila_valini['freg'].'</p>';
