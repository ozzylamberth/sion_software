...
class MYPDF extends TCPDF {
	public function Header() {
        ...
        // Title
        $this->Cell(180, 20, 'EPICRISIS', 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $this->Ln();
	...
	public function ColoredTable($header,$data,$data1) {
      ...
      $this->Ln();
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(25,5,'Estado Civil:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(25,5, utf8_encode($row['descripestadoc']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(20,5,'Ocupacion:',1,0,'C',1);
      $this->SetFont('helvetica', '',7);
      $this->Cell(110,5, utf8_encode($row['descriocu']),1,0,'C');
      $this->Ln();
      ...
      $this->Ln();
      $this->Cell(180,5,'Datos Acompañante:',1,0,'C',1);
      $this->Ln();
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(32,5,'Nombre acompañante:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(80,5, utf8_encode($row['nombre_aco']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(35,5,'Parentesco:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(33,5, utf8_encode($row['parentesco_aco']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Ln();
  		$this->Ln(10);
      $this->SetFont('helvetica', 'B', 8);
      ...
      $this->cell(35,0,$this->image($row['firma'] , $this->GetX(), $this->GetY(),35,20),0,'J');
    ...
    foreach($data1 as $row1) {
      ...
      $this->SetFont('','',7);
      $this->multiCell(180,0, utf8_encode($row1['justificacion_hosp']),1,'L');
      $this->SetFont('helvetica', 'B',8);
      $this->Cell(100,0,'Diagnostico principal:',1,0,'C',1);
      $this->Ln();
      $this->SetFont('helvetica', '',8);
      $this->multiCell(180,0, utf8_encode($row1['ddxp'].' --- '.$row1['tdxp']),1,'L');
      $this->SetFont('helvetica', 'B',8);
      $this->Cell(100,0,'Diagnostico Relacionado 1:',1,0,'C',1);
      $this->Ln();
      $this->multiCell(180,0, utf8_encode($row1['ddx1'].' --- '.$row1['tdx1']),1,'L');
      $this->SetFont('helvetica', 'B',8);
      $this->Cell(100,0,'Diagnostico Relacionado 2:',1,0,'C',1);
      $this->Ln();
      $this->multiCell(180,0, utf8_encode($row1['ddx2'].' --- '.$row1['tdx2']),1,'L');
      $this->SetFont('helvetica', 'B',8);
      $this->Cell(100,0,'Diagnostico Relacionado 3:',1,0,'C',1);
      $this->Ln();
      $this->multiCell(180,0, utf8_encode($row1['ddx3'].' --- '.$row1['tdx3']),1,'L');
      $this->cell(35,20,$this->image($row1['firma'] , $this->GetX(), $this->GetY(),35,20),0,'J');
      $this->Ln();
    }
...
b.id_adm_hosp,fingreso_hosp,hingreso_hosp,fegreso_hosp,hegreso_hosp,zona_residencia,nivel,tipo_servicio,resp_admhosp,
j.nom_eps,
k.id_evomed,'EPICRISIS' tipo_evo,k.freg_evomed fecha_evo,k.hreg_evomed hora_evo,justificacion_hosp,
dxp, ddxp, tdxp, dx1, ddx1, tdx1, dx2, ddx2, tdx2,
l.nombre,rm_profesional,l.especialidad espec_user,firma
...
where b.id_adm_hosp ='".$_GET["idadmhosp"]."' and k.tipo_evo='Epicrisis' ";
//echo $sql1;
