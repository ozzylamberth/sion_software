  ...
  $sede=$_REQUEST['sede'];
	$consulta = "SELECT a.tdoc_pac,doc_pac,nom_completo,fnacimiento,TIMESTAMPDIFF(YEAR,fnacimiento,CURDATE()) AS edad,
          ...
          group by a.tdoc_pac,doc_pac,nom_completo,fnacimiento,edad,
              fescala,hreg,total,calificacion_total,observacion,c.estado ,
              d.nombre
          ORDER BY total DESC ";
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Codedrinks") //Autor
							 ...
							 ->setTitle("INFORME CONSOLIDADO BARTHEL")
							 ->setSubject("INFORME CONSOLIDADO BARTHEL")
							 ->setDescription("INFORME CONSOLIDADO BARTHEL")
							 ->setKeywords("INFORME CONSOLIDADO BARTHEL")
							 ->setCategory("Reporte excel");

		$tituloReporte = "CONSOLIDADO BARTHEL";
		$titulosColumnas = array('PACIENTE','IDENTIFICACION','FECHA NACIMIENTO','EDAD','RESULTADO', 'TOTAL', 'OBSERVACION', 'JEFE','DIAGNOSTICO');
		...
		while ($fila = $resultado->fetch_array()) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E'.$i,  $fila['calificacion_total'])
                ->setCellValue('F'.$i,  $fila['total'])
                ->setCellValue('G'.$i,  utf8_encode($fila['observacion']))
		...
		$objPHPExcel->getActiveSheet()->setTitle('consolidadoBARTHEL');
		...
		header('Content-Disposition: attachment;filename="consolidadoBARTHEL.xlsx"');
