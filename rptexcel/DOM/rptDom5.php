  ...
  $sede=$_REQUEST['sede'];
	$consulta = "SELECT a.tdoc_pac,doc_pac,nom_completo,fnacimiento,TIMESTAMPDIFF(YEAR,fnacimiento,CURDATE()) AS edad,
          ...
          group by a.tdoc_pac,doc_pac,nom_completo,fnacimiento,edad,
              fescala,hreg,total,riesgo,observacion,c.estado ,
              d.nombre
          ORDER BY total DESC ";
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Codedrinks") //Autor
							 ...
							 ->setTitle("INFORME CONSOLIDADO NORTON")
							 ->setSubject("INFORME CONSOLIDADO NORTON")
							 ->setDescription("INFORME CONSOLIDADO NORTON")
							 ->setKeywords("INFORME CONSOLIDADO NORTON")
							 ->setCategory("Reporte excel");

		$tituloReporte = "CONSOLIDADO NORTON";
		...
		$objPHPExcel->getActiveSheet()->setTitle('consolidadoNORTON');
		...
		header('Content-Disposition: attachment;filename="consolidadoNORTON.xlsx"');
