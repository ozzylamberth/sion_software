...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data,$data1,$data2,$data3,$data4) {
      ...
      $this->Cell(60,0, utf8_encode('Turno: 3 Horas'),1,0,'C',1);
        $this->SetFont('','B',12);
      $this->Cell(120,0, utf8_encode('Fecha de realizacion: '.$row['freg3']),1,0,'C',1);
      $this->SetFont('','B',9);
      $this->Ln();
      $this->SetFont('','B',7);
      ...
      $this->Cell(15,0,utf8_encode($row['hnota3']) ,1,0,'C',0);
      $this->Cell(20,0, 'Hora final:',1,0,'C',1);
      $hnotaf = strtotime ( '+60 minute' , strtotime ( $row['hnota3'] ) ) ;
...
if ($turno==3) {
  if ($f1!='') {
    $sql="SELECT a.id_enf_dom3, id_adm_hosp,  freg_reg, freg3, hnota1, nota1, hnota2, nota2, hnota3, nota3, estado_nota, f_cancela, resp_cancela,
                 b.nombre,rm_profesional,firma,doc
          FROM enferdom3 a inner join user b on a.id_user=b.id_user
          WHERE a.id_adm_hosp=$adm AND estado_nota='Realizada' and id_d_aut_dom=$idd and a.freg3 BETWEEN '$f1' and '$f2'
          ORDER BY freg3 ASC
    ";
