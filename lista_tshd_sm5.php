...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			case 'VI':
				$sql="INSERT INTO tshd_sm ( id_adm_hosp, id_user, freg_tsce_sm, hreg_tsce_sm,tipo_atencion,
					h_familiar,estudio_socio,analisis,concepto_ts,recomendaciones,estado_tsce_sm) VALUES
				('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."',
				'".$_POST["tipoatencion"]."','".$_POST["h_familiar"]."','".$_POST["estudio_socio"]."','".$_POST["analisis"]."',
				'".$_POST["concepto_ts"]."','".$_POST["recomendaciones"]."','Realizada')";
				$subtitulo="Valoracion Hospital dia";
				$subtitulo1="Adicionado";
				$subtitulo2="Trabajo social";
			break;
			case 'EVO':
				$sql="INSERT INTO evo_tshd_sm (id_adm_hosp, id_user, freg_evohd, hreg_evohd,
					evolucion_hd, estado_evohd, resp_evohd) VALUES
				('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."',
				'".$_POST["evolucion"]."','Realizada','".$_SESSION["AUT"]["nombre"]."')";
				$subtitulo="Evolucion";
				$subtitulo1="Adicionado";
				$subtitulo2="Trabajo social";
			break;
...
if (isset($_GET["mante"])){					///nivel 2
	switch ($_GET["mante"]) {
		...
		where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$date=date('Y-m-d');
			$date1=date('H:i');
			$form1='formularios/val_tshd_sm.php';
			$subtitulo='Valoracion Trabajo social Hospital dia';
			break;
			case 'EVO':
			$sql="SELECT a.tdoc_pac,a.doc_pac,nom1,nom2,ape1,ape2,edad,fnacimiento,dir_pac,
			tel_pac,rh,email_pac,genero,lateralidad,religion,fotopac,
			...
			where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
				...
				$date=date('Y-m-d');
				$date1=date('H:i');
				$form1='formularios/evo_tshd_sm.php';
				$subtitulo='Evolucion Trabajo social Hospital Dia';
				break;
...
<section class="panel panel-default">
	...
	<section class="panel-heading"><h3>Hospital Dia Trabajo Social (Salud Mental)</h3></section>
	<section class="panel-body">
		<form>
		...
		<form>
			<section class="col-xs-12">
				...
				<article class="col-xs-3">
					...
					<input type="text" class="form-control" name="nom" placeholder="Digite nombre o apellido">
				</article>
				<div>
					<input type="submit" name="buscar" class="btn btn-primary" value="Consultar">
					<input type="hidden" name="opcion" Value="<?php echo $_GET["opcion"];?>"/>
	...
	<section class="panel-body">
		...
		<table class="table table-bordered table-responsive">
			...
			$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,fotopac,
						...
						WHERE p.doc_pac='".$doc."' and a.estado_adm_hosp='Activo' and tipo_servicio='Hospital dia'";
			...
			if ($tabla=$bd1->sub_tuplas($sql)){
				...
				foreach ($tabla as $fila ) {
					...
					echo'<th class="text-center">
								<a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VI&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-plus-circle"></span></button></a>
							 </th>';
					echo'<th class="text-center">
								<a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-stethoscope"></span></button></a>
							 </th>';
					echo "</tr>\n";
				}
			}
			...
			$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,fotopac,
						...
						FROM pacientes p LEFT JOIN adm_hospitalario a on p.id_paciente=a.id_paciente
														 LEFT JOIN sedes_ips s on a.id_sedes_ips=s.id_sedes_ips
						WHERE p.nom1 LIKE '%".$doc."%' and a.estado_adm_hosp='Activo'  and tipo_servicio='Hospital dia'";
