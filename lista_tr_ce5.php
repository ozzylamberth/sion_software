		...
		if($_POST["aceptar"]!="Descartar"){
			...
			switch ($_POST["operacion"]) {
				case 'VI':
					...
					'".$_POST["dx3"]."', '".$_POST["tdx3"]."', 'TERAPIA RESPIRATORIA','".$_POST["recomendaciones"]."', '".$_POST["plan_tratamiento"]."','Realizada')";
					$subtitulo="Valoracion inicial";
					$subtitulo1="Adicionado";
				break;
			case 'CONTROL':
				$sql="INSERT INTO evo_tr_ce (  id_adm_hosp, id_user, freg_evotr_ce, hreg_evotr_ce, evoluciontr_ce, estado_evotr_ce) VALUES
...
if (isset($_GET["mante"])){					///nivel 2
    ...
    where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
      ...
      where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$doc=$_REQUEST['doc'];
			$opcion=$_REQUEST['opcion'];
			$form1='formulariosCE/evo_tr_ce5.php';
...
<section class="panel-heading"><h4>TERAPIA RESPIRATORIA Consulta externa</h4></section>
	...
	if ($tabla=$bd1->sub_tuplas($sql)){
		...
		foreach ($tabla as $fila ) {
			...
			echo'<th class="text-right">';
						...
						$sql_valini="SELECT id_valini_ce,freg,tterapia FROM val_ini_ce WHERE id_adm_hosp=$adm and tterapia='TERAPIA RESPIRATORIA'
						LIMIT 1";
...
if (isset($_REQUEST["nom"])){
	...
	if ($tabla=$bd1->sub_tuplas($sql)){
		foreach ($tabla as $fila ) {
			...
			echo'<th class="text-right">';
						$adm=$fila['id_adm_hosp'];
						$sql_valini="SELECT id_valini_ce,freg FROM val_ini_ce WHERE id_adm_hosp=$adm and tterapia='TERAPIA RESPIRATORIA'
						LIMIT 1";
						if ($tabla_valini=$bd1->sub_tuplas($sql_valini)){
							foreach ($tabla_valini as $fila_valini ) {
								echo'<p>Valoración inicial registrada el '.$fila_valini['freg'].'</p>';
