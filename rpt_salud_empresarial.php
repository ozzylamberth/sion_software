...
class MYPDF extends TCPDF {
	public function Header() {
        ...
        // Title
        $this->Cell(180, 20, 'HISTORIA CLINICA', 1, false, 'R', 0, '', 0, false, 'M', 'M');
        $this->Ln();
    ...
    foreach($data1 as $row1) {
      ...
      $this->Cell(26,5,'Nombre Paciente:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(84,5, utf8_encode($row1['nom1']." ".$row1['nom2']." ".$row1['ape1']." ".$row1['ape2']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(35,5,'Documento Paciente:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(35,5, utf8_encode($row1['tdoc_pac'].': '.$row1['doc_pac']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Ln();
      $this->Cell(26,5,'F. Naciemiento:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(18,5, utf8_encode($row1['fnacimiento']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(12,5,'Edad:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(15,5, utf8_encode($row1['edad'].' '.$row1['descripuedad']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(7,5,'RH:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(7,5, utf8_encode($row1['rh']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(12,5,'Genero:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(17,5, utf8_encode($row1['genero']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(7,5,'Tel:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(59,5, utf8_encode($row1['tel_pac']),1,0,'C');
      $this->Ln();
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(15,5,'Direccion:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(80,5, utf8_encode($row1['dir_pac']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(15,5,'Email:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(70,5, utf8_encode($row1['email_pac']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Ln();
      $this->Cell(22,5,'Fecha Ingreso:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(32,5, utf8_encode($row1['fingreso_hosp'].' | '.$row1['hingreso_hosp']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(22,5,'Fecha Egreso:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(32,5, utf8_encode($row1['fegreso_hosp'].' | '.$row1['hegreso_hosp']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(8,5,'EPS:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(64,5, utf8_encode($row1['nom_eps']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Ln();
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(15,5,'T.Usuario:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(25,5, utf8_encode($row1['descritusuario']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(17,5,'T.Afiliacion:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(25,5, utf8_encode($row1['descriafiliado']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(22,5,'Departamento:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(30,5, utf8_encode($row1['descripdep']),1,0,'C');
      $this->SetFont('helvetica', 'B', 8);
      $this->Cell(16,5,'Municipio:',1,0,'C',1);
      $this->SetFont('helvetica', '',8);
      $this->Cell(30,5, utf8_encode($row1['descrimuni']),1,0,'C');
      $this->Ln(10);
      $this->Cell(40,6,'Fecha Registro:',1,0,'C',1);
      $this->Cell(50,6, utf8_encode($row1['fecha'].' | '.$row1['hora']),1,0,'C');
      $this->Ln();
      $this->multiCell(180,0,'SUBJETIVO :',1,'C');
      $this->multicell(180,0,utf8_encode($row1['subjetivo']),1,'J');
      $this->Cell(180,0,'OBJETIVO:',1,1,'C',1);
      $this->multicell(180,0,utf8_encode($row1['objetivo']),1,'J');
      $this->Cell(180,0,'ANALISIS:',1,0,'C',1);
      $this->Ln();
      $this->multicell(180, 0,utf8_encode($row1['analisis_evomed']), 1, 'J');
      $this->Cell(180,0,'PLAN TRATAMIENTO:',1,0,'C',1);
      $this->Ln();
      $this->multicell(180, 0,utf8_encode($row1['plan_tratamiento']), 1, 'J');
      $this->Cell(180,0,'DIAGNOSTICOS:',1,0,'C',1);
      $this->Ln();
      $this->multicell(180, 0,utf8_encode($row1['ddxp'] .' '.$row1['tdxp'] ), 1, 'J');
      $this->SetFont('','',1);
      $this->cell(70,20,$this->image(utf8_encode($row1['firma']) , $this->GetX(), $this->GetY(),30,20),1,'C');
      $this->SetFont('','B',9);
      $this->MultiCell(110,20, utf8_encode('Profesional:'.$row1['nombre'].' RM profesional:'.$row1['rm_profesional'].' Especialidad:'.$row1['espec_user']),1,'L');
...
i.descripuedad,
j.nom_eps,
k.id_hcsm_pv, freg_hcsm_pv, hreg_hcsm_pv, motivo_consulta, enfer_actual, his_personal, his_familiar, perso_premorbida, ant_alergicos, ant_patologico, ant_quirurgico, ant_toxicologico, ant_farmaco, ant_gineco, ant_psiquiatrico, ant_hospitalario, ant_traumatologico, ant_familiar, otros_ant, estado_general, tad, tas, tam, fr, fc, so, peso, talla, temperatura, imc, cabcuello, torax, ext, abdomen, neurologico, genitourinario, examen_mental, analisis, finalidad, causa_externa, dxp, ddxp, tdxp, dx1, ddx1, tdx1, dx2, ddx2, tdx2, dx3, ddx3, tdx3, plantratamiento, tipo_atencion, estado_hcsm_pv,
...
where b.id_adm_hosp ='".$_GET["idadmhosp"]."' and b.tipo_servicio='Salud Empresarial' and k.freg_hcsm_pv BETWEEN '$f1' and '$f2'";
//echo $sql;
$rs = mysql_query($sql);
...
i.descripuedad,
j.nom_eps,
k.freg_evomed	fecha,k.hreg_evomed hora,k.subjetivo subjetivo,k.objetivo objetivo, k.analisis_evomed,k.plan_tratamiento,k.ddxp,k.tdxp,k.ddx1,k.tdx1,k.ddx2,k.tdx2,k.id_user usuario,
l.nombre,rm_profesional,l.especialidad espec_user,firma
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      left join estado_civil c on (c.codestadoc = a.estadocivil)
      left join tusuario e on (e.codtusuario=b.tipo_usuario)
      ...
      left join uedad i on (i.coduedad=a.uedad)
      left join eps j on (j.id_eps=b.id_eps)
      left join evolucion_medica k on (k.id_adm_hosp=b.id_adm_hosp)
      left join user l on (l.id_user=k.id_user)
where b.id_adm_hosp ='".$_GET["idadmhosp"]."' and b.tipo_servicio='Salud Empresarial' and k.freg_evomed BETWEEN '$f1' and '$f2'
";
