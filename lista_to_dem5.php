...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			break;
			case 'EVO':
				$sql="INSERT INTO evo_to_dem (id_adm_hosp, id_user, freg_evoto, hreg_evoto, evolucionto, estado_evoto) VALUES
				('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."','".$_POST["evoto"]."','Realizada')";
				$subtitulo="Evolución Terapia ocupacional";
			...
			if ($_SESSION["AUT"]["especialidad"]=='ADMINISTRATIVO') {
				$sql="";
				$msg='LA ESPECIELIDAD DE SU PERFIL NO LE PERMITE REALIZAR REGISTROS ASISTENCIALES';
				$subtitulo="Plan Trimestral Individual Terapia ocupacional";
				$subtitulo1="Adicionado";
				$subtitulo2="Demencias";
...
//echo $sql;
		if($sql!=""){
			...
			"genero"=>"","lateralidad"=>"","religion"=>"","fotopac"=>"","id_adm_hosp"=>"","ideps"=>"","fingreso_hosp"=>"","hingreso_hosp"=>"","fegreso_hosp"=>"","hegreso_hosp"=>"",
			"nom_eps"=>"","id"=>"","fecha"=>"", "fvence_ptgen_dem"=>"", "objetivo"=>"", "estado_ptgen_dem"=>"");
			print_r($fila);
			}
		}else{
			$fila=array("tdoc_pac"=>"","doc_pac"=>"","nom1"=>"","nom2"=>"","ape1"=>"","ape2"=>"","edad"=>"","fnacimiento"=>"","dir_pac"=>"","tel_pac"=>"","rh"=>"","email_pac"=>"",
		...
		<script>
			function validar(){
				if (document.forms[0].hreg.value > document.forms[0].fecha.value){
					alert("Terapeuta ocupacional <?php echo $_SESSION["AUT"]["nombre"]?>, NO puede adelantar el tiempo.");
...
<section class="panel-heading"><h4>Terapia Ocupacional INDE Programa Demencias</h4></section>
