...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			case 'EVO':
				$sql="INSERT INTO nota_hd_sm (id_adm_hosp, id_user, freg_notahd, hreg_notahd, notahd, estado_notahd, resp_notahd) VALUES
				('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."','".$_POST["evolucion"]."','Realizada','".$_SESSION["AUT"]["nombre"]."')";
				$subtitulo="Registro ";
				$subtitulo1="Adicionado";
				$subtitulo2="Enfermería";
			break;
		}
		echo $sql;
		if ($bd1->consulta($sql)){
...
if (isset($_GET["mante"])){					///nivel 2
	switch ($_GET["mante"]) {
			...
			where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
				$boton="Agregar Registro enfermería";
				$atributo1=' readonly="readonly"';
				$atributo2='';
				$atributo3='';
				$date=date('Y-m-d');
				$date1=date('H:i');
				$form1='formularios/nota_hd_sm.php';
				$subtitulo='Registro de enfermería Hospital día';
				break;
...
<section class="panel panel-default">
	...
	<section class="panel-heading"><h3>Hospital Día ENFERMERÍA (Salud Mental)</h3></section>
	<section class="panel-body">
		<form>
		...
		<table class="table table-bordered table-responsive">
			...
			if ($tabla=$bd1->sub_tuplas($sql)){
				...
				foreach ($tabla as $fila ) {
					...
					echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-stethoscope"></span> Registro<br>enfermeria</button></a></th>';
					echo "</tr>\n";
				}
			}
			...
			$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,fotopac,
					  ...
					  FROM pacientes p LEFT JOIN adm_hospitalario a on p.id_paciente=a.id_paciente
									 LEFT JOIN sedes_ips s on a.id_sedes_ips=s.id_sedes_ips
						WHERE p.nom_completo LIKE '%".$doc."%' and a.estado_adm_hosp='Activo'  and tipo_servicio='Hospital dia'";
