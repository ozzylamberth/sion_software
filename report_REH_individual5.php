...
<div class="panel-body">
			          	...
			          	<input type="text" class="form-control" name="doc" placeholder="Digite Identificacion">
								...
								<article class="col-xs-3">
									...
									<select name="tterapia" class="form-control" <?php echo atributo3; ?>>
										...
										<option value="TERAPIA ASISTIDA POR PERROS">TERAPIA ASISTIDA POR PERROS</option>
	...
	<tr>
		...
		<th id="th-estilo4">Informes RS</th>
		<th id="th-estilo4">Evoluciones RS</th>
	</tr>
	...
	$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,edad,descricie,fotopac,a.id_adm_hosp,id_eps eps,fingreso_hosp,hingreso_hosp,s.nom_sedes,e.nom_eps FROM pacientes p LEFT JOIN adm_hospitalario a on p.id_paciente=a.id_paciente LEFT JOIN sedes_ips s on a.id_sedes_ips=s.id_sedes_ips LEFT JOIN eps e on a.id_eps=e.id_eps  WHERE p.doc_pac='".$doc."'  and tipo_servicio='Rehabilitacion' ";

	if ($tabla=$bd1->sub_tuplas($sql)){
		//echo $sql;
		foreach ($tabla as $fila ) {
			if ($fila['eps']==13 || $fila['eps']==16 || $fila['eps']==15) {
				echo"<tr >\n";
				...
				echo'<th class="text-center" ><a href="rpt_im_redservicios_inde.php?idadmhosp='.$fila["id_adm_hosp"].'&f1='.$f1.'&f2='.$f2.'&tterapia='.$tterapia.'&cie='.$cie.'"><button type="button" class="btn btn-success sombra_movil " ><span class="fa fa-file-pdf-o"></span></button></a></th>';
				echo'<th class="text-center" ><a href="rpt_evolucion_redservicios_inde.php?idadmhosp='.$fila["id_adm_hosp"].'&f1='.$f1.'&f2='.$f2.'&nom='.$fila["nom1"].' '.$fila["nom2"].' '.$fila["ape1"].' '.$fila["ape2"].'&edad='.$edad.'&cie='.$cie.'&tterapia='.$tterapia.'"><button type="button" class="btn btn-success sombra_movil " ><span class="fa fa-file-pdf-o"></span></button></a></th>';
				echo "</tr>\n";
			}else {
				echo"<tr >\n";
				echo'<td class="text-center">'.$fila["id_adm_hosp"].'</td>';
